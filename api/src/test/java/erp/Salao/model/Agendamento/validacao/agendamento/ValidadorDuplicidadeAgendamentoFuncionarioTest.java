...
class ValidadorDuplicidadeAgendamentoFuncionarioTest {
  ...
  @Test
  @DisplayName(
    "Cenário 1: Funcionário já possui agendamento no mesmo horário -> Deve lançar exceção"
  )
  void validar_cenario01() {
    ...
    when(
      agendamentoRepository.existsByFuncionarioIdAndDataAndMotivoCancelamentoIsNull(
        idFuncionario,
        data
      )
    ).thenReturn(true);
  ...
  @Test
  @DisplayName(
    "Cenário 2: Funcionário livre no horário -> Não deve lançar exceção"
  )

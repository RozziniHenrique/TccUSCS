...
public class FuncionarioService {
  ...
  public Funcionario cadastrar(CadastrarFuncionarioDTO dados) {
    ...
    String prefixoNome =
      dados.nome().length() >= 3 ? dados.nome().substring(0, 3) : dados.nome();
    ...
    if (
      dados.especialidadesIds() != null && !dados.especialidadesIds().isEmpty()
    ) {
      var especialidades = especialidadeRepository.findAllById(
        dados.especialidadesIds()
      );

      if (especialidades.size() != dados.especialidadesIds().size()) {
        throw new ValidacaoException(
          "Uma ou mais especialidades informadas são inválidas."
        );
      }
  ...
  public Page<ListarFuncionarioDTO> listar(
    ...
    return repository
      .buscarProfissionaisComFiltros(idEspecialidade, buscarAtivos, paginacao)
  ...
  public void removerEspecialidade(Long idFuncionario, Long idEspecialidade) {
    var funcionario = repository
      .findById(idFuncionario)
      .orElseThrow(() ->
        new EntityNotFoundException("Funcionário não encontrado")
      );
    var especialidade = especialidadeRepository
      .findById(idEspecialidade)
      .orElseThrow(() ->

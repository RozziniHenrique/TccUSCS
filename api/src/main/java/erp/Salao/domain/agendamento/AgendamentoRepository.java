...
{
  boolean existsByFuncionarioIdAndDataAndMotivoCancelamentoIsNull(
    Long idFuncionario,
    LocalDateTime data
  );

  boolean existsByClienteIdAndDataAndMotivoCancelamentoIsNull(
    Long idCliente,
    LocalDateTime data
  );
  ...
  @Query(
    """
        SELECT new erp.Salao.domain.dashboard.dto.RankingFuncionario(f.nome, COUNT(a))
        FROM Agendamento a
  ...
  @Query(
    """
        SELECT new erp.Salao.domain.dashboard.dto.AlertaQualidade(f.nome, AVG(a.nota))
        FROM Agendamento a
        JOIN a.funcionario f
  ...
  @Query(
    "SELECT a FROM Agendamento a WHERE a.funcionario.id = :id OR a.cliente.id = :id"
  )
  Page<Agendamento> buscaPersonalizada(Long id, Pageable paginacao);

  @Query(
    value = "SELECT c.nome as nome, SUM(e.preco) as totalGasto, MAX(a.data) as ultimaVisita " +
      "FROM clientes c " +
